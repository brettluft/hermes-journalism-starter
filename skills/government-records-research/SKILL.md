...
Run the newsroom configuration CLI `status` command, then read `$HERMES_HOME/newsroom/newsroom.json` and `$HERMES_HOME/newsroom/sources.json` as the current authoritative files. Do not rely on remembered configuration. Check setup status, scope, preferences, source status, authority status, record-type checks, and validation timestamps. Before searching or writing a result, require `newsroom.editorial_style.confirmed_by_editor` to be `true`; if it is absent or false, do not produce the research report and invoke `newsroom-setup` to obtain editor confirmation. Apply the confirmed guide and house rules to the report.

If either file is missing or invalid, or setup status is `draft` or `partial`, invoke or recommend `newsroom-setup`. Both `draft` and `partial` are unfinished and force the final status `search incomplete`. In other words, draft and partial setup states never permit a complete research status. The user may explicitly request narrowly scoped ad hoc research when setup is unfinished. Ad hoc scope never relaxes the confirmed-style prerequisite: proceed only after a fresh authoritative configuration read confirms `newsroom.editorial_style.confirmed_by_editor=true`. Then use a visible coverage warning, define what will be checked, and still use `search incomplete` while source setup is unfinished. If configuration is missing or invalid, or style is absent or unconfirmed, stop and run newsroom setup before reporting.
